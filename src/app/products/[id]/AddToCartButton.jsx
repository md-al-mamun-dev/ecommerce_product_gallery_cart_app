...
export default function AddToCartButton({product}) {
  ...
  return (
    <div className="w-full mt-6  items-center flex flex-col md:flex-row-reverse gap-4 justify-between">
        <button onClick={()=>dispatch(addToCart(product))} className=" w-full md:w-2/3  bg-red-600 hover:bg-red-700 text-white font-semibold py-3 px-6 rounded-xl transition">
            Add to Cart
        </button>
        {
            items.find(item => item.id === product.id) 
            &&  <button onClick={()=>dispatch(removeFromCart(product.id))} className="flex w-full md:w-1/4 lg:w-1/2 justify-center items-center gap-2 px-6 py-3 rounded-xl border border-gray-200 bg-white hover:border-red-400 active:scale-[0.98] transition-all duration-200 group">
                    <svg 
                        width="24" 
                        height="24" 
                        viewBox="0 0 24 24" 
